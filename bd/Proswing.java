...
public class Proswing {
	...
	public Proswing() throws SQLException {
	...
	private void initialize() throws SQLException {
		...
		panel.setBorder(BorderFactory.createTitledBorder(
                BorderFactory.createEtchedBorder(), "Registro de Provincias"));
		...
		panel_1.setBorder(BorderFactory.createTitledBorder(
                BorderFactory.createEtchedBorder(), "Tabla_Provincia"));
		...
		try (Connection myConn = DriverManager.getConnection(dbURL, username, password)){					
			...
			ResultSet myRs = myStmt.executeQuery("SELECT * FROM gzz_provincia;");	
			while(myRs.next()) {
				String ProCod=myRs.getString("ProCod");
				String ProDes=myRs.getString("ProDes");
				String ProEstReg=myRs.getString("ProEstReg");
								
				String []tbData= {ProCod,ProDes,ProEstReg};						
		...
		lblPaises = new JLabel("PROVINCIAS");
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String ProCod = textField.getText();
				String ProDes = textField_1.getText();
				String ProEstReg = textField_2.getText();				
				if (!isInteger(ProCod)||ProCod.equals("")||ProDes.equals("")||ProEstReg.equals("")||(!ProEstReg.equals("I")&&!ProEstReg.equals("A"))) {
					JOptionPane.showMessageDialog(panel, "Datos erróneos");
				}else {
				try (Connection myConn = DriverManager.getConnection(dbURL, username, password)){					
					Statement myStmt = myConn.createStatement();
					myStmt.executeUpdate("INSERT INTO `gzz_provincia` (`ProCod`, `ProDes`, `ProEstReg`)  VALUES ('"+ProCod+"','"+ProDes+"','"+ProEstReg+"');");					
				} catch (SQLException e1) {
					e1.printStackTrace();
				}
		...
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String Procodigo = table.getModel().getValueAt(row, column).toString();
				...
				try (Connection myConn = DriverManager.getConnection(dbURL, username, password)){					
					Statement myStmt = myConn.createStatement();
					myStmt.executeUpdate("UPDATE gzz_provincia SET ProCod ='"+ProCod+"',ProDes ='"+ProDes+"',ProEstReg ='"+ProEstReg+"' WHERE ProCod = "+Procodigo+";");					
				} catch (SQLException e1) {
					e1.printStackTrace();
				}
			...
			public void actionPerformed(ActionEvent e) {
				...
				switch (result) {
		         case JOptionPane.YES_OPTION:
		        	column = 0;
					row = table.getSelectedRow();
					String ProCod = table.getModel().getValueAt(row, column).toString();
					
		        	 try (Connection myConn = DriverManager.getConnection(dbURL, username, password)){					
							Statement myStmt = myConn.createStatement();
							myStmt.executeUpdate("DELETE FROM gzz_provincia WHERE ProCod = '"+ProCod+"';");					
						} catch (SQLException e1) {
							e1.printStackTrace();
						}		        	 
		...
		btnInactivar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try (Connection myConn = DriverManager.getConnection(dbURL, username, password)){					
					Statement myStmt = myConn.createStatement();
					myStmt.executeUpdate("UPDATE gzz_provincia SET ProEstReg ='I' WHERE ProCod = "+ProCod+";");					
				} catch (SQLException e1) {
					e1.printStackTrace();
				}
		...
		btnInactivar_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try (Connection myConn = DriverManager.getConnection(dbURL, username, password)){					
					Statement myStmt = myConn.createStatement();
					myStmt.executeUpdate("UPDATE gzz_provincia SET ProEstReg ='A' WHERE ProCod = "+ProCod+";");					
				} catch (SQLException e1) {
					e1.printStackTrace();
				}
			...
			public void actionPerformed(ActionEvent e) {				
				try (Connection myConn = DriverManager.getConnection(dbURL, username, password)){					
					...
					while(myRs.next()) {
						...
						System.out.println(myRs.getString("ProCod") + 
								", " + myRs.getString("ProDes")+ 
								", "+myRs.getString("ProEstReg") );						
						String []tbData= {ProCod,ProDes,ProEstReg};	
						tblModel.addRow(tbData);					
					}
