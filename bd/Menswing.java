...
public class Menswing {
	...
	public Menswing() throws SQLException {
	...
	private void initialize() throws SQLException {
		...
		panel.setBorder(BorderFactory.createTitledBorder(
                BorderFactory.createEtchedBorder(), "Registro de Mentolado"));
		...
		panel_1.setBorder(BorderFactory.createTitledBorder(
                BorderFactory.createEtchedBorder(), "Tabla_Mentolado"));
		...
		try (Connection myConn = DriverManager.getConnection(dbURL, username, password)){					
			...
			ResultSet myRs = myStmt.executeQuery("SELECT * FROM gzl_mentolado;");	
			while(myRs.next()) {
				String CodMen=myRs.getString("CodMen");
				String DesMenCig=myRs.getString("DesMenCig");
				String EstMen=myRs.getString("EstMen");
								
				String []tbData= {CodMen,DesMenCig,EstMen};						
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String CodMen = textField.getText();
				String DesMenCig = textField_1.getText();
				String EstMen = textField_2.getText();				
				if (!isInteger(CodMen)||CodMen.equals("")||DesMenCig.equals("")||EstMen.equals("")||(!EstMen.equals("I")&&!EstMen.equals("A"))) {
					JOptionPane.showMessageDialog(panel, "Datos erróneos");
				}else {
				try (Connection myConn = DriverManager.getConnection(dbURL, username, password)){					
					Statement myStmt = myConn.createStatement();
					myStmt.executeUpdate("INSERT INTO `gzl_mentolado` (`CodMen`, `DesMenCig`, `EstMen`)  VALUES ('"+CodMen+"','"+DesMenCig+"','"+EstMen+"');");					
				} catch (SQLException e1) {
					e1.printStackTrace();
				}
		...
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String Mencodigo = table.getModel().getValueAt(row, column).toString();
				...
				try (Connection myConn = DriverManager.getConnection(dbURL, username, password)){					
					Statement myStmt = myConn.createStatement();
					myStmt.executeUpdate("UPDATE gzl_mentolado SET CodMen ='"+CodMen+"',DesMenCig ='"+DesMenCig+"',EstMen ='"+EstMen+"' WHERE CodMen = "+Mencodigo+";");					
				} catch (SQLException e1) {
					e1.printStackTrace();
				}
			...
			public void actionPerformed(ActionEvent e) {
				...
				switch (result) {
		         case JOptionPane.YES_OPTION:
		        	column = 0;
					row = table.getSelectedRow();
					String CodMen = table.getModel().getValueAt(row, column).toString();
					
		        	 try (Connection myConn = DriverManager.getConnection(dbURL, username, password)){					
							Statement myStmt = myConn.createStatement();
							myStmt.executeUpdate("DELETE FROM gzl_mentolado WHERE CodMen = '"+CodMen+"';");					
						} catch (SQLException e1) {
							e1.printStackTrace();
						}		        	 
		...
		btnInactivar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try (Connection myConn = DriverManager.getConnection(dbURL, username, password)){					
					Statement myStmt = myConn.createStatement();
					myStmt.executeUpdate("UPDATE gzl_mentolado SET EstMen ='I' WHERE CodMen = "+CodMen+";");					
				} catch (SQLException e1) {
					e1.printStackTrace();
				}
		...
		btnInactivar_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try (Connection myConn = DriverManager.getConnection(dbURL, username, password)){					
					Statement myStmt = myConn.createStatement();
					myStmt.executeUpdate("UPDATE gzl_mentolado SET EstMen ='A' WHERE CodMen = "+CodMen+";");					
				} catch (SQLException e1) {
					e1.printStackTrace();
				}
			...
			public void actionPerformed(ActionEvent e) {				
				try (Connection myConn = DriverManager.getConnection(dbURL, username, password)){					
					...
					while(myRs.next()) {
						...
						System.out.println(myRs.getString("CodMen") + 
								", " + myRs.getString("DesMenCig")+ 
								", "+myRs.getString("EstMen") );						
						String []tbData= {CodMen,DesMenCig,EstMen};	
						tblModel.addRow(tbData);					
					}
