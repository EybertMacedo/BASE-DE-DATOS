...
public class Claswing {
	...
	public Claswing() throws SQLException {
	...
	private void initialize() throws SQLException {
		...
		panel.setBorder(BorderFactory.createTitledBorder(
                BorderFactory.createEtchedBorder(), "Registro de Clases de Cigarrillo"));
		...
		panel_1.setBorder(BorderFactory.createTitledBorder(
                BorderFactory.createEtchedBorder(), "Tabla_Clase"));
		...
		try (Connection myConn = DriverManager.getConnection(dbURL, username, password)){					
			...
			ResultSet myRs = myStmt.executeQuery("SELECT * FROM gzl_clase;");	
			while(myRs.next()) {
				String CodCla=myRs.getString("CodCla");
				String DesColCla=myRs.getString("DesColCla");
				String ClaCig=myRs.getString("ClaCig");
								
				String []tbData= {CodCla,DesColCla,ClaCig};						
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String CodCla = textField.getText();
				String DesColCla = textField_1.getText();
				String ClaCig = textField_2.getText();				
				if (!isInteger(CodCla)||CodCla.equals("")||DesColCla.equals("")||ClaCig.equals("")||(!ClaCig.equals("I")&&!ClaCig.equals("A"))) {
					JOptionPane.showMessageDialog(panel, "Datos erróneos");
				}else {
				try (Connection myConn = DriverManager.getConnection(dbURL, username, password)){					
					Statement myStmt = myConn.createStatement();
					myStmt.executeUpdate("INSERT INTO `gzl_clase` (`CodCla`, `DesColCla`, `ClaCig`)  VALUES ('"+CodCla+"','"+DesColCla+"','"+ClaCig+"');");					
				} catch (SQLException e1) {
					e1.printStackTrace();
				}
		...
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String Clacodigo = table.getModel().getValueAt(row, column).toString();
				...
				try (Connection myConn = DriverManager.getConnection(dbURL, username, password)){					
					Statement myStmt = myConn.createStatement();
					myStmt.executeUpdate("UPDATE gzl_clase SET CodCla ='"+CodCla+"',DesColCla ='"+DesColCla+"',ClaCig ='"+ClaCig+"' WHERE CodCla = "+Clacodigo+";");					
				} catch (SQLException e1) {
					e1.printStackTrace();
				}
			...
			public void actionPerformed(ActionEvent e) {
				...
				switch (result) {
		         case JOptionPane.YES_OPTION:
		        	column = 0;
					row = table.getSelectedRow();
					String CodCla = table.getModel().getValueAt(row, column).toString();
					
		        	 try (Connection myConn = DriverManager.getConnection(dbURL, username, password)){					
							Statement myStmt = myConn.createStatement();
							myStmt.executeUpdate("DELETE FROM gzl_clase WHERE CodCla = '"+CodCla+"';");					
						} catch (SQLException e1) {
							e1.printStackTrace();
						}		        	 
		...
		btnInactivar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try (Connection myConn = DriverManager.getConnection(dbURL, username, password)){					
					Statement myStmt = myConn.createStatement();
					myStmt.executeUpdate("UPDATE gzl_clase SET ClaCig ='I' WHERE CodCla = "+CodCla+";");					
				} catch (SQLException e1) {
					e1.printStackTrace();
				}
		...
		btnInactivar_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try (Connection myConn = DriverManager.getConnection(dbURL, username, password)){					
					Statement myStmt = myConn.createStatement();
					myStmt.executeUpdate("UPDATE gzl_clase SET ClaCig ='A' WHERE CodCla = "+CodCla+";");					
				} catch (SQLException e1) {
					e1.printStackTrace();
				}
			...
			public void actionPerformed(ActionEvent e) {				
				try (Connection myConn = DriverManager.getConnection(dbURL, username, password)){					
					...
					while(myRs.next()) {
						...
						System.out.println(myRs.getString("CodCla") + 
								", " + myRs.getString("DesColCla")+ 
								", "+myRs.getString("ClaCig") );						
						String []tbData= {CodCla,DesColCla,ClaCig};	
						tblModel.addRow(tbData);					
					}
