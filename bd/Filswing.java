...
public class Filswing {
	...
	public Filswing() throws SQLException {
	...
	private void initialize() throws SQLException {
		...
		panel.setBorder(BorderFactory.createTitledBorder(
                BorderFactory.createEtchedBorder(), "Registro de Filtros"));
		...
		panel_1.setBorder(BorderFactory.createTitledBorder(
                BorderFactory.createEtchedBorder(), "Tabla_Filtros"));
		...
		try (Connection myConn = DriverManager.getConnection(dbURL, username, password)){					
			...
			ResultSet myRs = myStmt.executeQuery("SELECT * FROM gzl_filtro;");	
			while(myRs.next()) {
				String CodFil=myRs.getString("CodFil");
				String DesFilCig=myRs.getString("DesFilCig");
				String EstFil=myRs.getString("EstFil");
								
				String []tbData= {CodFil,DesFilCig,EstFil};						
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String CodFil = textField.getText();
				String DesFilCig = textField_1.getText();
				String EstFil = textField_2.getText();				
				if (!isInteger(CodFil)||CodFil.equals("")||DesFilCig.equals("")||EstFil.equals("")||(!EstFil.equals("I")&&!EstFil.equals("A"))) {
					JOptionPane.showMessageDialog(panel, "Datos erróneos");
				}else {
				try (Connection myConn = DriverManager.getConnection(dbURL, username, password)){					
					Statement myStmt = myConn.createStatement();
					myStmt.executeUpdate("INSERT INTO `gzl_filtro` (`CodFil`, `DesFilCig`, `EstFil`)  VALUES ('"+CodFil+"','"+DesFilCig+"','"+EstFil+"');");					
				} catch (SQLException e1) {
					e1.printStackTrace();
				}
		...
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try (Connection myConn = DriverManager.getConnection(dbURL, username, password)){					
					Statement myStmt = myConn.createStatement();
					myStmt.executeUpdate("UPDATE gzl_filtro SET CodFil ='"+CodFil+"',DesFilCig ='"+DesFilCig+"',EstFil ='"+EstFil+"' WHERE CodFil = "+Colcodigo+";");					
				} catch (SQLException e1) {
					e1.printStackTrace();
				}
			...
			public void actionPerformed(ActionEvent e) {				
				...
				switch (result) {
		         case JOptionPane.YES_OPTION:
		        	column = 0;
					row = table.getSelectedRow();
					String CodFil = table.getModel().getValueAt(row, column).toString();
					
		        	 try (Connection myConn = DriverManager.getConnection(dbURL, username, password)){					
							Statement myStmt = myConn.createStatement();
							myStmt.executeUpdate("DELETE FROM gzl_filtro WHERE CodFil = '"+CodFil+"';");					
						} catch (SQLException e1) {
							e1.printStackTrace();
						}		        	 
		...
		btnInactivar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try (Connection myConn = DriverManager.getConnection(dbURL, username, password)){					
					Statement myStmt = myConn.createStatement();
					myStmt.executeUpdate("UPDATE gzl_filtro SET EstFil ='I' WHERE CodFil = "+CodFil+";");					
				} catch (SQLException e1) {
					e1.printStackTrace();
				}
		...
		btnInactivar_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try (Connection myConn = DriverManager.getConnection(dbURL, username, password)){					
					Statement myStmt = myConn.createStatement();
					myStmt.executeUpdate("UPDATE gzl_filtro SET EstFil ='A' WHERE CodFil = "+CodFil+";");					
				} catch (SQLException e1) {
					e1.printStackTrace();
				}
			...
			public void actionPerformed(ActionEvent e) {				
				try (Connection myConn = DriverManager.getConnection(dbURL, username, password)){					
					...
					while(myRs.next()) {
						...
						System.out.println(myRs.getString("CodFil") + 
								", " + myRs.getString("DesFilCig")+ 
								", "+myRs.getString("EstFil") );						
						String []tbData= {CodFil,DesFilCig,EstFil};	
						tblModel.addRow(tbData);					
					}
