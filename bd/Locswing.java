...
public class Locswing {
	...
	public Locswing() throws SQLException {
	...
	private void initialize() throws SQLException {
		...
		panel.setBorder(BorderFactory.createTitledBorder(
                BorderFactory.createEtchedBorder(), "Registro de Localidades"));
		...
		panel_1.setBorder(BorderFactory.createTitledBorder(
                BorderFactory.createEtchedBorder(), "Tabla_Localidades"));
		...
		try (Connection myConn = DriverManager.getConnection(dbURL, username, password)){					
			...
			ResultSet myRs = myStmt.executeQuery("SELECT * FROM gzz_localidad;");	
			while(myRs.next()) {
				String LocCod=myRs.getString("LocCod");
				String LocDes=myRs.getString("LocDes");
				String LocEstReg=myRs.getString("LocEstReg");
								
				String []tbData= {LocCod,LocDes,LocEstReg};						
		...
		lblPaises = new JLabel("LOCALIDAD");
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String LocCod = textField.getText();
				String LocDes = textField_1.getText();
				String LocEstReg = textField_2.getText();				
				if (!isInteger(LocCod)||LocCod.equals("")||LocDes.equals("")||LocEstReg.equals("")||(!LocEstReg.equals("I")&&!LocEstReg.equals("A"))) {
					JOptionPane.showMessageDialog(panel, "Datos erróneos");
				}else {
				try (Connection myConn = DriverManager.getConnection(dbURL, username, password)){					
					Statement myStmt = myConn.createStatement();
					myStmt.executeUpdate("INSERT INTO `gzz_localidad` (`LocCod`, `LocDes`, `LocEstReg`)  VALUES ('"+LocCod+"','"+LocDes+"','"+LocEstReg+"');");					
				} catch (SQLException e1) {
					e1.printStackTrace();
				}
		...
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String tablePaiEstReg = table.getModel().getValueAt(row, column).toString();
				if(tablePaiEstReg.equals("I")) {
					JOptionPane.showMessageDialog(panel, "El registro está inactivo, no se puede modificar");
				}else {
				column = 0;				
				row = table.getSelectedRow();
				String Loccodigo = table.getModel().getValueAt(row, column).toString();
				...
				try (Connection myConn = DriverManager.getConnection(dbURL, username, password)){					
					Statement myStmt = myConn.createStatement();
					myStmt.executeUpdate("UPDATE gzz_localidad SET LocCod ='"+LocCod+"',LocDes ='"+LocDes+"',LocEstReg ='"+LocEstReg+"' WHERE LocCod = "+Loccodigo+";");					
				} catch (SQLException e1) {
					e1.printStackTrace();
				}
		...
		btnNewButton_2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String tablePaiEstReg = table.getModel().getValueAt(row, column).toString();
				if(tablePaiEstReg.equals("I")) {
					JOptionPane.showMessageDialog(panel, "El registro está inactivo, no se puede eliminar");
				}else {
				...
				switch (result) {
		         case JOptionPane.YES_OPTION:
		        	column = 0;
					row = table.getSelectedRow();
					String LocCod = table.getModel().getValueAt(row, column).toString();
					
		        	 try (Connection myConn = DriverManager.getConnection(dbURL, username, password)){					
							Statement myStmt = myConn.createStatement();
							myStmt.executeUpdate("DELETE FROM gzz_localidad WHERE LocCod = '"+LocCod+"';");					
						} catch (SQLException e1) {
							e1.printStackTrace();
						}		        	 
		...
		btnInactivar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try (Connection myConn = DriverManager.getConnection(dbURL, username, password)){					
					Statement myStmt = myConn.createStatement();
					myStmt.executeUpdate("UPDATE gzz_localidad SET LocEstReg ='I' WHERE LocCod = "+LocCod+";");					
				} catch (SQLException e1) {
					e1.printStackTrace();
				}
		...
		btnInactivar_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try (Connection myConn = DriverManager.getConnection(dbURL, username, password)){					
					Statement myStmt = myConn.createStatement();
					myStmt.executeUpdate("UPDATE gzz_localidad SET LocEstReg ='A' WHERE LocCod = "+LocCod+";");					
				} catch (SQLException e1) {
					e1.printStackTrace();
				}
			...
			public void actionPerformed(ActionEvent e) {				
				try (Connection myConn = DriverManager.getConnection(dbURL, username, password)){					
					...
					while(myRs.next()) {
						...
						System.out.println(myRs.getString("LocCod") + 
								", " + myRs.getString("LocDes")+ 
								", "+myRs.getString("LocEstReg") );						
						String []tbData= {LocCod,LocDes,LocEstReg};	
						tblModel.addRow(tbData);					
					}
